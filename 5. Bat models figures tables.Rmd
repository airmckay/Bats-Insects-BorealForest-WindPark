...
# Overview of survey effort across facilities - no met tower 
active.night = 'TRUE'
bats_allnight <- bats_gb_simple %>% 
  mutate( night = as.Date(night))

bats_allnight$active.night <- active.night
bats_allnight  <- bats_allnight %>% mutate(bats.active = 
              case_when(Batpass < 1 ~ "No", Batpass >= 1 ~ "Yes")) 
bats_allnight$bats.active <- as.factor(bats_allnight$bats.active)
...
survey.nights <- dplyr::count(survey.effort, Site)

bats_gb_trim1 <- left_join(bats_gb_trim, survey.nights) %>% dplyr::rename(survey.nights = n)
...
## Table results - Summary of bat datasets 
...
# met tower - seasonal
met$activenight <- "Active"
met$night <- as.Date(met$night)

metplot<-
  ggplot() + geom_count(data = met1, aes(x=night, y=guild, color = guild)) + 
  geom_point(data = met, aes(x = night, y = activenight), color = "black", shape = 17, alpha = 0.75, size = 3) + 
...
plot_grid(groundplot, metplot, nrow = 2)
...
CI(met1$wind_mean, 0.90)
#    upper     mean    lower 
# 6.248783 5.853254 5.457725 

CI(met1$temp_mean, 0.90)
#    upper     mean    lower 
# 17.24277 16.62713 16.01149 

met.test <- met1 %>% filter(temp_mean > 13 & wind_mean < 10) 
# 65/72 = 0.9027778

# When was 90% of bat activity at the met tower? 
summary(met1)
test.temp <- met1 %>% filter(temp_mean > 13.3)
test.wind <- met1 %>% filter(wind_mean < 9)

quantile(met1$temp_mean, probs = 0.9)
quantile(met1$wind_mean, probs = 0.9)
...
bats_tot$night <- as.Date(bats_tot$night)
maxbats <- bats_tot %>% filter(Batpass_sum > 100) %>% droplevels
summary(maxbats)

summary(maxbats$wind_mean)
  #  Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
  # 2.571   3.866   4.795   5.222   6.046   8.890 

summary(maxbats$temp_mean)
  #  Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
  # 13.47   15.09   16.01   17.63   20.85   24.58 

maxbats_plot <-
  ggplot(maxbats) + 
  geom_point(aes(x = night, y = Batpass_sum), color = "black", alpha = 0.8, size = 3) +
  geom_smooth(aes(x = night, y = Batpass_sum), color = "grey") +
  theme(panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(),
        text = element_text(size = 13),
panel.background = element_blank(), axis.line = element_line(colour = "black"), legend.position="none") +
  xlab("") + ylab("Bat passes per night")

wind_plot <- ggplot(maxbats) + geom_smooth(aes (x = night, y = wind_mean), color = "blue", alpha = 0.3) +
  theme(panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(),
        text = element_text(size = 13),
panel.background = element_blank(), axis.line = element_line(colour = "black"), legend.position="none") +
  xlab("") + ylab("Mean wind speed (m/s)")
  
temp_plot <- ggplot(maxbats) + geom_smooth(aes ( x = night, y = temp_mean), color = "red", alpha = 0.3)+
  theme(panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(),
        text = element_text(size = 13),
panel.background = element_blank(), axis.line = element_line(colour = "black"), legend.position="none") +
  xlab("") + ylab("Mean nightly temperature (\u00B0C)")

plot_grid(maxbats_plot, temp_plot, wind_plot, nrow = 3, labels = c("a.", "b.", "c.")) 


### Same plots but for all met tower data 
...
wind_plot <- ggplot(met) + geom_smooth(aes (x = night, y = wind_mean), color = "blue", alpha = 0.3) +
  theme(panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(),
        text = element_text(size = 13),
panel.background = element_blank(), axis.line = element_line(colour = "black"), legend.position="none") +
...
temp_plot <- ggplot(met) + geom_smooth(aes ( x = night, y = temp_mean), color = "red", alpha = 0.3)+
  theme(panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(),
        text = element_text(size = 13),
panel.background = element_blank(), axis.line = element_line(colour = "black"), legend.position="none") +
