...
dataset1 <- "1_Marker2020_ManualID_withnoise_original.csv"

path1 <- str_c(input, "/", dataset1)


manual_output <- read_csv(path1) #257516 passes with noise
...
mo <- mo %>% select(-1) %>% select(-c(FOLDER, CHANNEL, OFFSET, OUT.FILE.FS, OUT.FILE.ZC, ORGID, USERID, REVIEW.ORGID, REVIEW.USERID, INPATHMD5, OUTPATHMD5FS, OUTPATHMD5ZC)) # Columns full of NAs or not relevant to the workflow 

names.mo <- as.data.frame(names(mo))
# write.csv(names.mo, file = file.path(output_today, "cleaed_manual.output_names_27.11.2023"))
# write.csv(mo, file = file.path(output_today, "cleaed_manual.output_27.11.2023"))
